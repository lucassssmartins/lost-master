...
public class StoreMenu extends MenuInventory {
                        ...
                        public void onClick(Player p, Inventory inv, ClickType type, ItemStack stack, int slot) {
                            ...
                            if (p.hasPermission("pvpkit2." + ability.getName().toLowerCase())) {
                                ...
                                if (profile.getData(DataType.COINS).getAsInt() >= ability.getBuyPrice()) {
                                    ...
                                    for (int i = 1; i <= 2; i++) {
                                        PermissionProduct permission = new PermissionProduct("hgkit" + i + "." + ability.getName(),
                                        ...
                                        }.runTaskLater(HungerGames.getInstance(), (i * 1L));
                                    }
                                    profile.save();
    ...
    public List<Kit> getAbilities(Player player) {
        List<Kit> kits = new ArrayList<>(Management.getManagement(KitManager.class).getAllKits());
        kits.removeIf(e -> e instanceof Nenhum);
        return kits;
    }
