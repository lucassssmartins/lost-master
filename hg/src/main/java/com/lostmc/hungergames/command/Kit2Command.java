...
public class Kit2Command extends WrappedBukkitCommand {
    ...
    public void execute(WrappedCommandSender sender, String label, String[] args) {
        if (sender.getHandle() instanceof Player) {
            ...
            if (hg.getGameMode().isDoubleKit()) {
                if (!hg.getGameStage().isGametime() && !hg.getGameStage().isEnding()) {
                    if (hg.getGameStage().isPregame() || (hg.getGameStage().isInvincibility() && !gamer.hasSecondaryKit())
                        ...
                        if (args.length == 0) {
                            ...
                            if (kit != null) {
                                if (!gamer.hasPrimaryKit() || !kit.isIncompatible(gamer.getPrimaryKit())) {
                                    if (kit.isActived()) {
                                        if (manager.hasPrimaryKit(p, kit)) {
                                            if (kit instanceof Nenhum || (!gamer.hasSecondaryKit() || !gamer.getSecondaryKit().equals(kit))
                                                    && (!gamer.hasPrimaryKit() || !gamer.getPrimaryKit().equals(kit))) {
                                                if (gamer.isAlive()) {
                                            ...
                                            p.sendMessage("§eVocê não possui o kit §9" + kit.getName()
                                                    + "§e secundário, adquira já em §9loja.lostmc.com.br");
                                        }
                                    } else {
                ...
                p.sendMessage("§cEste torneio não é DoubleKit.");
            }
        } else {
