		...
		try {
			if (!PacketAccessor.isLegacyVersion()) {
				if (!PacketAccessor.isParamsVersion()) {
					typeEnumChatFormat = (Class<? extends Enum>) Class
				...
				Class<?> typeCraftChatMessage = Class
			...
			try {
				if (PacketAccessor.isLegacyVersion()) {
					...
					if (!PacketAccessor.isParamsVersion()) {
						...
						PacketAccessor.DISPLAY_NAME.set(packetParams,
								Array.get(CraftChatMessage.invoke(null, name), 0));
